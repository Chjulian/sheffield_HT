...
mydata <- read.csv("data/wave2-metadata-20210427.csv",
                   stringsAsFactors =F)
...
#################################
wards.patients<- read.csv("data/wave2-movements-20210421.csv")
...
dna <- read.FASTA("data/wave2-seqs-rm-errors-masked.aln")
mylabels <- clean_data(as.data.frame(names(dna)))
names(dna) <- mylabels$names_dna
dna <- dna[mydata$barcode]; mylabels <- names(dna)
write.FASTA(dna, paste0("data/", output.name,  "wave2-seqs-rm-errors-masked.ord.aln"))
dna.SNP <- import_fasta_sparse(paste0("data/", output.name,  "wave2-seqs-rm-errors-masked.ord.aln")) %>% snp_dist(.)
rownames(dna.SNP) <- colnames(dna.SNP) <- mylabels; rm(mylabels)
unlink(paste0("data/", output.name,  "wave2-seqs-rm-errors-masked.ord.aln"))
...
##remove 3 locations with in out errors
wards$ok <- wards$end > wards$start
wards <- wards[wards$ok==TRUE,]
wards$ok <- NULL
...
config <- create_config(
  ...
  move_mu = TRUE, init_mu = 0.2, sd_mu = 0.05
)
...
configs <- assign_imports(n_chains = 5, p_import_mu = 0.6, p_import_sd = 0.06, pre_run, ll, config)
...
##read in bay links
baylink.df <- read.csv("data/bay_link_all_Wave2.csv", header = T, stringsAsFactors = F)
...
summary.df <- lapply(seq(1, length(res.list)), function(i){
  ...
  step.summary <- analyse_run(loop.steps[loop.steps$step==steps[1],], mydata, baylink.pairs, "wave2")
  for (step in steps[2:length(steps)]){
    summary.row <- analyse_run(loop.steps[loop.steps$step==step,], mydata, baylink.pairs, "wave2")   
