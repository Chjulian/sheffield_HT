...
mydata <- read.csv("data/wave2-metadata-20210421.csv",
                   stringsAsFactors =F)
...
summary.df <- lapply(seq(1, length(res.list)), function(i){
  ...
  loop.steps <- add_meta(loop.steps, mydata, data)
  ...
  step.summary <- analyse_run(loop.steps[loop.steps$step==steps[1],], mydata, baylink.pairs)
  for (step in steps[2:length(steps)]){
    summary.row <- analyse_run(loop.steps[loop.steps$step==step,], mydata, baylink.pairs)   
