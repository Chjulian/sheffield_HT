...
d.onset.sensitivity <- data.frame(
        'run'=integer(), 'max_dist'=integer(),
        'size.mean'=numeric(),
        'size.median'=numeric(),
        'size.max'=numeric(),
        "staff_to_staff"=numeric(),
        "inpatient_to_inpatient"=numeric(),
        "staff_to_inpatient"=numeric(),    
        "inpatient_to_staff"=numeric(),
        'duration.mean'=numeric(),
        'duration.median'=numeric(),
        'duration.max'=numeric(),
...
for(n.run in 1:nruns){
        ...
        d.row  <- data.frame(
                'run'=n.run, 'max_dist'=max_dist,
                'size.mean'=mean(tChains.df$size),
                'size.median'=median(tChains.df$size),
                'size.max'=max(tChains.df$size),
                "staff_to_staff"=sum(tChains.df$staff_to_staff),
                "inpatient_to_inpatient"=sum(tChains.df$inpatient_to_inpatient),
                "staff_to_inpatient"=sum(tChains.df$staff_to_inpatient),    
                "inpatient_to_staff"=sum(tChains.df$inpatient_to_staff),
                'duration.mean'=mean(tChains.df$duration),
                'duration.median'=median(tChains.df$duration),
                'duration.max'=max(tChains.df$duration),
                "index.category.inpatient"=as.vector(table(tChains.df$index.category)['inpatient']),
                "index.category.staff"=as.vector(table(tChains.df$index.category)['staff']))
        d.onset.sensitivity <- rbind(d.onset.sensitivity,d.row); rm(d.row)
        write.csv(d.onset.sensitivity,"output/d.onset.sensitivity.csv")
