...
pp <-
  as.numeric(table(paste(tpairs$from, tpairs$to, sep = "_")) / nrow(res))
hist(pp)
summary(pp)
...
potential_known <-
  lapply(unique(diff_ward_known$to), get_potential, data)
...
diff_ward_unknown <- filter(tpairs,
                            kappa == 1 &
                              ((
...
potential_check <-
  names(potential_unknown)[sapply(potential_unknown, nrow) > 0] %>%
  setNames(., .) %>%
  lapply(function(i) {
    ...
    tibble(
      ...
      already_ancestor = sapply(potents,
                                function(potent)
                                  sum(res[[glue("alpha_{i}")]] == potent, na.rm = TRUE) / nrow(res)),
      ## this checks if the proposed ancestor has been accepted as the infectee in the MCMC
      ancestor_is_infectee = sapply(potents,
                                    function(potent)
                                      sum(res[[glue("alpha_{potent}")]] == i, na.rm = TRUE) / nrow(res)),
