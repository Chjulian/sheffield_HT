...
onsetdistribution <- function(data = "mydata") {
  ...
  data <- data_read %>%
    ...
    mutate(
      HealthcareAssociation = ifelse(
        HealthcareAssociation == "Not admitted-Community Associated",
        "Not Admitted-Community Associated",
        HealthcareAssociation
      )
    )
  ...
  cat_names <-
    cat_names[!(cat_names == "EXTERNAL" | cat_names == "HHCON")]
  data_bycat <- purrr::map(.x = cat_names,
  ...
  for (pcat in loop.cats) {
    ...
    type_names_assigned <-
      unique(to_impute_comp$HealthcareAssociation)
    
    type_names_intersection <-
      intersect(type_names_toimpute, type_names_assigned)
    type_names_unabletobeimputated <-
      setdiff(type_names_toimpute, type_names_intersection)
    
    # reassign those unable to be imputed as 'undefined'
    to_impute <- to_impute %>%
      mutate(
        HealthcareAssociation = ifelse(
          HealthcareAssociation %in% type_names_unabletobeimputated,
          "undefined",
          HealthcareAssociation
        )
      )
    ...
    data_by_type_toimpute <- purrr::map(.x = type_names_intersection,
                                        ~ dplyr::filter(to_impute, HealthcareAssociation == .x)) %>%
      setNames(type_names_intersection)
    
    data_by_type_toimpute_comp <- purrr::map(.x = type_names_intersection,
    ...
    data_by_type_withvals <- purrr::map(.x = type_names_intersection,
    ...
    impute_vals <- purrr::map(
      ...
      ~ sample(
        x = data_by_type_withvals[[.x]]$Delay_OnsetSwab_forAnalysis,
        size = as.numeric(number_to_impute[number_to_impute$HealthcareAssociation ==
                                             .x, "n"]),
        replace = TRUE
      )
    ) %>%
    ...
    out_partial_bycat <- purrr::map(.x = type_names_intersection,
                                    ~ rbindlist(list(
                                      data_by_type_toimpute_comp[[.x]], imputed_data[[.x]]
                                    )))
