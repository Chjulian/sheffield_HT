...
## timed contact data
get_initial_tree <- function(data,
                             config,
  ...
  tmp_config <- create_config(
    ...
    init_pi = 1,
    move_pi = FALSE,
    init_eps = ifelse(config$init_eps == 1, 1, 0.9),
    move_eps = FALSE,
    n_iter_import = 2e3,
    init_tree = 'star',
    sample_every = 100,
  ...
  first_run <-
    outbreaker(data, tmp_config, priors = priors, likelihoods = likelihoods)
  ...
  n <- nrow(first_run)
  nm <- names(first_run)
  tmp_config$init_alpha <-
    tmp_config$init_tree <- unlist(first_run[n, grep('alpha', nm)])
  ...
  ## case
  to_fix <-
    tmp_config$init_t_onw - tmp_config$init_t_inf[tmp_config$init_tree] == 0
  ...
  nm <- names(sec_run)
  config$init_alpha <-
    config$init_tree <- unlist(sec_run[n, grep('alpha', nm)])
  ...
  fn[data$D[cbind(seq_along(config$init_alpha), config$init_alpha)] > max_dist] <-
    TRUE
  ...
  cat(paste0("Identified ", sum(is.na(
    config$init_alpha
  )),
...
get_ward_fn <- function(x) {
  calc_fn <- function(from, to, t_inf, t_onw, kappa, eps, tau) {
    ...
    if (kappa == 1) {
      ind1 <- t_inf + C_ind + 1
      if (ind1 < 1 | ind1 > ncol(data$ctd_timed_matrix))
        return(1)
      ...
      if (eps == 1) {
        if (data$ctd_timed_matrix[from, ind1] == data$ctd_timed_matrix[to, ind1] &
            data$ctd_timed_matrix[to, ind1] != -1)
          return(0)
        else
          return(1)
      } else {
      ...
      if (ind1 < 1 | ind1 > ncol(data$ctd_timed_matrix))
        return(1)
      if (ind2 < 1 | ind1 > ncol(data$ctd_timed_matrix))
        return(1)
      ...
      if (tau == 1 & eps == 1) {
        if (data$ctd_timed_matrix[from, ind1] == data$ctd_timed_matrix[to, ind2] &
            data$ctd_timed_matrix[to, ind2] != -1)
          return(0)
        else
          return(1)
      } else {
  ...
  out <-
    mapply(calc_fn, from, to, t_inf, t_onw, kappa, MoreArgs = list(eps, tau))
...
vis_ward <-
  function(res,
    ...
    df2$date_from[df2$kappa > 1 &
                    !is.na(df2$kappa)] <- t_onw[df2$kappa > 1 & !is.na(df2$kappa)]
    df2 %<>%
      mutate(
...
get_ward_summary <- function(res, data) {
  ...
  ward_match <-
    unique(data$ctd_timed[, 2])[!is.na(unique(data$ctd_timed[, 2]))]
  
  lapply(seq_len(nrow(res)), get_summary, res) %>%
    bind_rows() %>%
    mutate(across(c(ward_from, ward_to), ~ ifelse(. == 0, "Unknown", ward_match[.])),
...
prune_cycles <- function(i,
  ...
  if (parent$from %in% cycle_edges$to) {
    ...
    edge_remove <-
      cycle_edges[which.min(cycle_edges[[rank_contact]]), ]
...
get_coor <- function(x,
  ...
  if (!rank_contact %in% names(contacts)) {
    ...
    weight <-
      linelist[[rank_contact]][to_ind] - linelist[[rank_contact]][from_ind]
  ...
  for (i in seq_len(max(depth))) {
    ...
    if (i > 1) {
      ...
      pos[ind, 1:(i - 1)] <-
        pos[match(parent[ind], linelist$id), 1:(i - 1)]
      ...
      if (length(multiple_parents) > 0) {
        ...
        linelist$cluster_member <-
          as.numeric(linelist$cluster_member)
        ...
        get_combn <-
          function(j)
            if (length(unique(j)) > 1)
              t(utils::combn(unique(j), 2))
        linked_roots <- lapply(linked, get_combn)
        linked_roots <-
          linked_roots[!vapply(linked_roots, is.null, TRUE)]
      ...
      if (!is.null(root_order)) {
        ...
        ord <-
          order(linelist[[root_order]][ind], decreasing = reverse_root_order)
        ...
        if (length(linked_roots) > 0) {
          ...
          for (j in seq_len(nrow(linked_roots))) {
            ...
            min_ind <-
              match(linked_roots[j, which.min(sub_ord)], linelist$id[ind])
            max_ind <-
              match(linked_roots[j, which.max(sub_ord)], linelist$id[ind])
  ...
  if (!is.null(igraph_type)) {
    ...
    if (igraph_type == 'rt') {
      abs_pos <-
        igraph::layout.reingold.tilford(net, root = which(depth == 1))[, 1]
    } else if (igraph_type == 'sugiyama') {
      abs_pos <-
        igraph::layout.sugiyama(net, layers = x$linelist[[x_axis]])$layout[, 1]
...
rescale <- function(x,
  ...
  if (length(unique(x)) == 1L)
    return(x)
  out <-
    (x - min(x, na.rm = TRUE)) / (diff(range(x, na.rm = TRUE))) * (max_val - min_val)
  out <- out + min_val
  return(out)
}
...
get_mindist <- function(dist) {
  sapply(seq_len(nrow(dist)), function(x)
    min(dist[x,-x]))
}
...
update_par <- function(param, i, res, data, config) {
  ...
  param$kappa <-
    as.integer(as.vector(unl(res[i, grep('kappa', names(res))])))
...
update_trans <- function(data, param, config) {
  for (i in seq_along(param$trans_mat)) {
    param$trans_mat[[i]] <-
      outbreaker2:::get_transition_mat(
        data$pp_trans_adj[[i]],
        data$pp_place[[i]],
        data$pp_place_adj[[i]],
        param$eps[i],
...
get_res_mindist <- function(res, dist) {
  sapply(seq_along(res[grep("alpha", names(res))]),
         function(i)
           min(dist[i, unique(res[grep("alpha", names(res))][[i]])]))
}
...
get_potential_old <- function(i, dist, dates) {
  ...
  if (length(potential) == 0)
    return(numeric())
  potential[sapply(potential,
                   function(x)
                     any(get_ward(
                       x, (data$dates[i] - 30):(data$dates[i] - 1), data
                     ) != -1))]
}

## extract transmission pair information
get_tpairs <- function(res, data) {
  lapply(seq_len(sum(grepl(
    "alpha", names(res)
  ))),
  function(i) {
    tibble(
...
get_potential <- function(i, data, max_dist = 0) {
  ...
  matches <-
    apply(potential_wards, 2, function(x)
      x == wards & !x %in% c(-1, 0))
...
ll_pois_i <- function(data, param, i = NULL) {
  if (is.null(i))
    i <- seq_along(param$alpha)
  sum(dpois(data$D[cbind(param$alpha[i], i)],
            mu * param$kappa[i],
            log = TRUE),
      na.rm = TRUE)
}
...
get_overlap <- function(start, end) {
  map_lgl(seq_along(start),
          function(i)
            any(start[i] > end[-i] & start[-i] > end[i]))
}
...
correct_dates <- function(df) {
  if (nrow(df) > 1) {
    ...
    if (sum(single) > 1)
      warning(
        paste0(
          "resolution is too small to include all data points; ",
...
scale_distribution <-
  function(density, scale = 6)
    rep(density / scale, each = scale)

## calculate the individual likelihood for all cases for a given step in the MCMC
get_step_likelihood <-
  function(step, res, data, param, config, likelihoods) {
    param <- update_par(param, step, res, data, config)
    ll <-
      map_dbl(seq_along(data$dates),
              ~ outbreaker2:::cpp_ll_all(data, param, .x, likelihoods))
...
assign_imports <-
  function(n_chains,
    ...
    if (p_import_mu < prop_fixed) {
      stop(
        paste0(
          "p_import_mu is lower than the minimum necessary number of imports",
    ...
    ## = p_import_mu
    par <-
      get_beta_par((p_import_mu - prop_fixed) / (1 - prop_fixed), p_import_sd)
    ...
    n_import <-
      map_dbl(p_import, ~ rbinom(1, length(ll) - sum(is.na(config$init_tree)), .x))
    ...
    unfixed_ll <-
      exp(-ll[unfixed]) %>% modify_if( ~ .x > max_ll, ~ max_ll)
    ...
    imports <-
      map(n_import, ~ sample(which(unfixed), .x, FALSE, unfixed_ll))
    
    ## set last tree of res as first tree in config
    param <-
      update_par(create_param()$current, nrow(res), res, data, config)
...
run_and_save <-
  function(config,
           ...
           prefix) {
    res <-
      outbreaker2:::outbreaker(data, config, priors = priors, likelihoods = likelihoods)
    export(res, here(glue("./output/{prefix}_run_{i}.rds")))
    return(res)
  }
