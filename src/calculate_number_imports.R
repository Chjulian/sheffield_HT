...
data <- data.table::fread(paste0("./data/", file_name))
message("Using ", file_name, " as input")

# incubation period as Lognormal
dist_setup <- function(dist_mean = NULL,
                       dist_sd = NULL) {
...
inpatient_data <- data %>%
  ...
  mutate(Prob_CommunityAcquired = ifelse(
    Delay_AdmissionOnset > 0,
...
save(
  number_imports_average,
  number_imports_loCI,
  number_imports_hiCI,
  proportion_imports_average,
  proportion_imports_loCI,
  proportion_imports_hiCI,
  file = paste("./output/NumberImports_", file_name, ".Rda")
)
