add_meta <- function(run, mydata, data, scaled_date_onset, origin, scale){
        ...
        run$t_inf <- as.Date(run$t_inf, origin = min(scaled_date_onset))
        run$t_inf <- scale_onset_dates(run$t_inf, origin, to = "down", scale = scale)
        run$from.onset <- mydata$dateofonset_foranalysis[match(run$from, mydata$barcode)]
        run$to.onset <- mydata$dateofonset_foranalysis[match(run$to, mydata$barcode)]
        run$incubation <- difftime(run$to.onset, run$t_inf, units = "days")
        run$serial.interval <- difftime(run$to.onset, run$from.onset,  units = "days")
        return(run)
}
...
analyse_run <- function(run, mydata, baylink.pairs, wave){
        ...
        network.all$unsampled.case <- ifelse(!is.na(network.all$kappa), network.all$kappa -1, network.all$kappa)  
        ...
        cluster.index.hoci <- sapply(unique(tree$linelist$cluster_member),
                                function(cluster){
                                        cluster.df <- network.all[network.all$cluster==cluster,]
                                        cluster.df <- cluster.df %>% arrange(t_inf)
                                        cluster.index <- cluster.df$from.hosp.assoc[1]
                                        return(cluster.index)
                                }
        ) 
        ...
        cluster.index.hoci.prop <- prop.table(table(cluster.index.hoci))
        ...
        cluster.size <- sapply(unique(tree$linelist$cluster_member),
                               function(cluster){
                                       size <- length(tree$linelist$cluster_member[
                                               ...
                                               sum(network.all$unsampled.case[network.all$cluster==cluster])
        ...
        secondary.infections <- left_join(secondary.infections,
                                          dplyr:::select(mydata, "barcode", "category", 
                                                         "healthcareassociation","dateofonset_foranalysis"), by="barcode")
        ...
        sec.inf.coca.quantile <- quantile(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_community_associated"])
        sec.inf.cosha.quantile <- quantile(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_suspected_healthcare_associated"])
        sec.inf.hoha.quantile <- quantile(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_healthcare_associated"])
        sec.inf.hoiha.quantile <- quantile(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_indetermite_healthcare_associated"])
        sec.inf.hosha.quantile <- quantile(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_suspected_healthcare_associated"])
        ...
        if (wave=="wave2"){
                top10.wards <-
                c("loc0080", "loc0123", "loc0200", "loc0116", "loc0088", 
                  "loc0113", "loc0032", "loc0124", "loc0112", "loc0033")
        }
        ...
        ward.outcomes <- lapply(top10.wards, function(i){
                ...
                n.staff <- length(loop.df$to[loop.df$to.cat=="staff"])
                n.pt <- length(loop.df$to[loop.df$to.cat=="inpatient"])
        ...
        wards.transmissions <- spread(select(ward.outcomes, "ward", "ward.transmission.n"), ward, ward.transmission.n)
        names(wards.transmissions) <- paste0(names(wards.transmissions), ".transmissions.n")
        wards.clusters <- spread(select(ward.outcomes, "ward", "ward.clusters.n"), ward, ward.clusters.n)
        names(wards.clusters) <- paste0(names(wards.clusters), ".clusters.n")
        wards.staff.n <- spread(select(ward.outcomes, "ward", "staff.n"), ward, staff.n)
        names(wards.staff.n) <- paste0(names(wards.staff.n), ".staff.n")
        wards.pt.n <- spread(select(ward.outcomes, "ward", "patient.n"), ward, patient.n)
        names(wards.pt.n) <- paste0(names(wards.pt.n), ".patient.n")
        wards.staff.prop <- spread(select(ward.outcomes, "ward", "staff.prop"), ward, staff.prop)
        names(wards.staff.prop) <- paste0(names(wards.staff.prop), ".staff.prop")
        wards.pt.prop <- spread(select(ward.outcomes, "ward", "patient.prop"), ward, patient.prop)
        names(wards.pt.prop) <- paste0(names(wards.pt.prop), ".patient.prop")
        ...
        out.tib <-  tibble(
                ...
                sec.inf.coca.mean = mean(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_community_associated"]),
                sec.inf.cosha.mean = mean(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_suspected_healthcare_associated"]),
                sec.inf.hoha.mean = mean(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_healthcare_associated"]),
                sec.inf.hoiha.mean = mean(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_indetermite_healthcare_associated"]),
                sec.inf.hosha.mean = mean(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_suspected_healthcare_associated"]),
                ...
                sec.inf.coca.sd = sd(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_community_associated"]),
                sec.inf.cosha.sd = sd(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_suspected_healthcare_associated"]),
                sec.inf.hoha.sd = sd(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_healthcare_associated"]),
                sec.inf.hoiha.sd = sd(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_indetermite_healthcare_associated"]),
                sec.inf.hosha.sd = sd(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_suspected_healthcare_associated"]),
                ...
                sec.inf.coca.median = median(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_community_associated"]),
                sec.inf.cosha.median = median(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="community_onset_suspected_healthcare_associated"]),
                sec.inf.hoha.median = median(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_healthcare_associated"]),
                sec.inf.hoiha.median = median(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_indetermite_healthcare_associated"]),
                sec.inf.hosha.median = median(secondary.infections$secondary.infections[secondary.infections$healthcareassociation=="hospital_onset_suspected_healthcare_associated"]),
                ...
                sec.inf.0.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==0]),
                sec.inf.1.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==1]),
                sec.inf.2.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==2]),
                sec.inf.3.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==3]),
                sec.inf.4.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==4]),
                sec.inf.5.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==5]),
                sec.inf.6.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==6]),
                sec.inf.7.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==7]),
                sec.inf.8.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==8]),
                sec.inf.9.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==9]),
                sec.inf.10.n = length(secondary.infections$barcode[secondary.infections$secondary.infections==10])
                )
        
        out.tib <- cbind(out.tib, 
                         wards.transmissions, 
                         wards.clusters,
                         wards.staff.n,
                         wards.staff.prop,
